...
function askForAction(cmd: CmdApi): JsonCmdAction {
  const typea = await cmd.ask(
    'What is the type of this action? (say/do)',
    fail
  );
...
function askForCommand(cmd: CmdApi): JsonCommand {
  ...
  const rank = Number.parseInt(
    await cmd.ask('What is the minimum rank to use this command? (1-3)', fail)
  );
  ...
  while (true) {
    ...
    const onans = await cmd.ask(
      'When would you like this command to be triggered? (ran)',
      fail
    );
  ...
  defineCommand(
    ...
    async (cmd) => {
      ...
      if (cmd.channel.startsWith('#'))
        throw 'Sorry, createpkg is only allowed in PMs to prevent spam.';
      cmd.respond(
        'Please note: only ASCII characters are allowed in these fields'
      );
      const name = await cmd.ask(
        'What would you like to name your package?',
        fail
      );
      ...
      while (true) {
        ...
        commands.push(askForCommand(cmd));
      }
      ...
      cmd.respond(
        `Your package has been stored in /home/hyper/hyperbot/pkgs/${pkgid}`
      );
      cmd.respond('Load it using the loadpkgstr command');
      const submit = await cmd.confirm(
        'Would you like to submit this package to be reviewed and potentially added to the bot?'
      );
      const hpmSubmit = await cmd.confirm(
        'Would you like to submit this package to HPM (hyper package manager)'
      );

      if (submit) {
        writeFileSync(
          `${config.jsonpkg.submissionrootdir}/submit-${pkgid}.pkg`,
          pkgstr
        );
        writeFileSync(
          `${config.jsonpkg.submissionrootdir}/submit-${pkgid}.json`,
          json
        );
        cmd.respond('Submitted!');
        Logger.info(
          'Package',
          `Submitted package ${pkgid} for review in ${config.jsonpkg.submissionrootdir}`
        );
      } else cmd.respond('Your package has not been submitted');
      if (hpmSubmit) {
        const user = await cmd.identify();
        const name = await cmd.ask(
          'Please enter a name for your HPM package',
          () => {
